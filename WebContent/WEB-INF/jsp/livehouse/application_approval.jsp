...
<body>
	...
	<main class="application-approval-main">
		...
		<c:if test="${cogigOrSolo == 1}">
			<section class="application-approval-section">
				<ul class="application-approval-ul-1">
					<li class="approved-artist-img-li"><c:set var="groupId"
							value="${application.groupId}" /> <c:set var="imagePath"
							value="${pictureImageMap[groupId]}" /> <c:if
							test="${not empty imagePath}">
		...
		<div class="application-approval-backhome-btn-div">
			<a href="<%=request.getContextPath()%>/Livehouse_home"
				class="application-approval-backhome-btn">ホームへ戻る</a>
		</div>
	</main>
</body>
</html>
